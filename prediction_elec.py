...
#判断函数,预警信号判断
def make_signal(a,b):
    if a < 26.818076610565:
        return 0
    elif a >=26.818076610565 and b >= 0.15:
        return 1
    else:
        return 0
'''
...
#设置数字标签
plt.text(-4.8,0.45, r'$\mu=-1.776,\ \sigma=0.833$')
...
df2 = df.query('signal==1')
#print (df2.head(10))
per = len(df2.signal)/len(df.signal)
print ('%.2f'%per)
#☆☆☆想按时间升序排列(但未实现)
...
#print (df.I_mean.corr(df.I_unbalance_percent))#相关系数
...
#因为样本很大,所以t分布趋向于正态分布 寻找电流阈值
#CI = stats.norm.interval(0.90, loc=mu, scale=sigma)
#test0 = mu + 1.96*sigma #答案一致0.95
#test = mu + 1.65*sigma #答案一致0.90
#print (CI[1]) #CI[1]代表右临界值
#df1 = df[['ds','I_mean','I_unbalance_percent','log']]
#df1 = df1[df.log > CI[1]]
#print (df1.head(15))
#print(df1.I_mean.max())
...
#prophet.plot(forecast,xlabel = 'Date', ylabel = 'I_ave').savefig('I_ave_original.png')
#prophet.plot(forecast,xlabel = 'Date', ylabel = 'adjusted_signal').show()
prophet.plot(forecast,xlabel = 'Date', ylabel = 'Adjusted_signal').savefig('I_unbalance_percent_Adjusted_signal.png')
#prophet.plot(forecast,xlabel = 'Date', ylabel = 'Original_Signal').savefig('Original_Signal.png')
