...
for a,b in zip(bins_a,n_a):
    plt.text(a,b + 0.0025,'%.1f%%'%(b/np.sum(n_a)*100),ha='left', va= 'bottom',fontsize=6)
#拟合Ua
y_a = stats.norm.pdf(bins_a, mu[0], sigma[0])
norm_a = plt.plot(bins_a, y_a*1000, color='gold',linestyle='--',label='norm')
#画参考线
...
for a,b in zip(bins_b,n_b):
    plt.text(a,b + 0.0025,'%.1f%%'%(b/np.sum(n_b)*100),ha='left', va= 'bottom',fontsize=6)
#拟合Ub
y_b = stats.norm.pdf(bins_b, mu[0], sigma[0])
norm_b = plt.plot(bins_b, y_b*1000, 'g--',label='norm')
#画参考线
...
for a,b in zip(bins_a,n_a):
    plt.text(a,b + 0.001,'%.1f%%'%(b/np.sum(n_c)*100),ha='left', va= 'bottom',fontsize=6)
#拟合Uc
y_c = stats.norm.pdf(bins_c, mu[0], sigma[0])
norm_c = plt.plot(bins_c, y_c*1000, 'r--',label='norm')
#画参考线
